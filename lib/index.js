...
import { PROVIDER, MODEL, MODEL_NAME, createGeminiWebAdapter } from './adapter.js';
...
export function apply(ctx) {
  ...
  // ── LLM adapter: register "gemini-bridge" provider so the model picker
  // offers a Gemini (Web / Pro) model that accepts pasted images. Selection
  // routes all conversation calls to the logged-in Gemini web app.
  const llmSvc = ctx.llm;
  if (llmSvc !== undefined) {
    try {
      llmSvc.registerConfigurableProviders([{
        provider: PROVIDER,
        displayName: 'Gemini (Web)',
        settingsNs: 'dsh-gemini-bridge',
        settingsPath: [],
      }]);
    } catch (e) {
      console.log('dsh-gemini-bridge: registerConfigurableProviders skipped: ' + (e instanceof Error ? e.message : String(e)));
    }
    const adapter = createGeminiWebAdapter(ctx.attachments);
    llmSvc.registerAdapter([PROVIDER], adapter);
    console.log('dsh-gemini-bridge: registered LLM provider "' + PROVIDER + '" / model "' + MODEL + '" (' + MODEL_NAME + ')');
  }
  ...
  // ── agent/pre-step: turn pasted/attached image blocks into Gemini text ──
  // When the current model cannot see images (text-only), replace each image
  // block in an incoming user message with a Gemini-generated description, so
  // pasting a screenshot/image into the composer "just works".
  ...
  if (attachments !== undefined && llm !== undefined) {
    ctx.on('agent/pre-step', async (payload, next) => {
      ...
      const hasImage = messages.some((m) => m && Array.isArray(m.content) && m.content.some((b) => b && b.type === 'image'));
      if (!hasImage) return decision;

      // If the current model already supports image input, leave blocks intact.
      try {
        const header = payload.agent && payload.agent.session ? payload.agent.session.requestHeader() : undefined;
        const cfg = header && header.config ? header.config : undefined;
        if (cfg && typeof cfg.provider === 'string' && typeof cfg.model === 'string') {
          const info = await llm.resolveModelInfo(cfg.provider, cfg.model, payload.signal);
          if (Array.isArray(info.inputModalities) && info.inputModalities.includes('image')) {
            return decision;
          }
        }
      } catch (error) {
        console.log('dsh-gemini-bridge: model image check failed, assuming text-only: ' + (error instanceof Error ? error.message : String(error)));
      }
      ...
      for (const message of messages) {
        ...
        for (const block of content) {
          ...
          let description;
          try {
            description = await recognizeWithGemini(attachments, block.attachment, payload);
          } catch (error) {
            description = '[image recognition failed: ' + (error instanceof Error ? error.message : String(error)) + ']';
          }
          nextContent.push({ type: 'text', text: '[image content] ' + description });
        }
        transformed.push({ id: message.id, role: message.role, source: message.source, content: nextContent });
      }
      return { kind: 'enter', messages: transformed };
    });
  }

  console.log('dsh-gemini-bridge: gemini_ask / gemini_vision / gemini_screen / gemini_chats / gemini_chat_read + paste-to-Gemini registered');
}

// Read an image attachment, write it to a temp file, ask Gemini (via the
// bridge) to describe it, and return the description text.
async function recognizeWithGemini(attachments, attachmentRef, payload) {
  const stored = await attachments.readImage(attachmentRef, payload.signal);
  ...
  const tmpFile = path.join(os.tmpdir(), 'dsh-gemini-paste-' + Date.now() + '-' + Math.random().toString(36).slice(2) + ext);
  writeFileSync(tmpFile, Buffer.from(stored.data));
  try {
    const syntheticExec = {
      agent: payload.agent,
      signal: payload.signal,
    };
    const data = await runBridge('vision', {
      image_path: tmpFile,
      prompt: 'Please describe this image in detail, including all visible text, objects, colors and layout.',
    }, syntheticExec);
    return data && data.text ? data.text : '[no description]';
  } finally {
